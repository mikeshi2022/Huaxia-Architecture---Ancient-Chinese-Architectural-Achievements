...
1. **本地开发（推荐）**：用 **HTTP** 打开整站，例如 VS Code **Live Server**、或在**本目录**（`华夏营造-中国古代建筑成就`）执行 `npx serve`，再访问终端里给出的地址（如 `http://localhost:3000/` 或 `http://localhost:3000/index.html`）。  
   - 若使用 **`npx serve`**：仓库内已提供 **`serve.json`**：`cleanUrls: false` 避免 `detail.html?…`、`craft-detail.html?…` 被重写成无后缀 URL 时**丢失查询参数**；`directoryListing: false` 关闭根目录文件列表；`rewrites` 将 **`/` 内部重写为 `/index.html`**，避免仅访问 `/` 时出现 404。请在包含 `serve.json` 的目录下执行 `npx serve`，修改配置后需重启进程。  
   - 亦可用 `python -m http.server 8080` 等，无上述重定向问题。  
...
| `js/detail-link-persist.js`                                            | 跳转详情前暂存建筑名，规避部分静态服务器丢 `?name=` |
